    ...
    if (countAiQuotes >= 3) {
      ...
      * Continúa de forma muy atenta la conversación y ofrécete a resolver cualquier duda general que tenga sobre el servicio, los filtros de seguridad, la app de reportes diarios, o el respaldo psicopedagógico.`;
      ...
      if (calculatedPrice) {
        ...
        * REGLA DE PRESENTACIÓN DE TARIFA EN TEXTO PLANO (CRÍTICO): Debes mencionar e integrar la tarifa estimada en el texto plano de tu respuesta (siguiendo el formato de las plantillas sugeridas). Para ello, tienes ESTRICTAMENTE PROHIBIDO inventar, deducir o leer montos de los tabuladores por tu cuenta. Debes usar **única y exclusivamente** el valor exacto determinado por el sistema: **$${calculatedPrice.toLocaleString("es-MX")} MXN** (y si corresponde a un servicio fijo recurrente de varios días a la semana, especifica que es una tarifa semanal/por semana).`;
      } else {
...
- diasSolicitados: Días de la semana, cantidad de días requerida, o fechas/días específicos del servicio (ej: "Lunes a Viernes", "3 días a la semana"). 
  CRÍTICO - CONTEO DE DÍAS OBLIGATORIO: Debes analizar el texto y calcular el número total de días de servicio solicitados. Guarda el texto descriptivo y OBLIGATORIAMENTE agrega al final el número de días calculado entre paréntesis con el formato "(X días)" o "(1 día)" (ej: "3 veces por semana (3 días)", "Lunes a Viernes (5 días)", "Lunes y Jueves (2 días)", "1 de septiembre (1 día)", "del 1 al 5 de septiembre (5 días)", "3 días a la semana (3 días)"). Esto es estrictamente obligatorio para todos los casos, de modo que el sistema de cotización pueda calcular el precio correcto. Si se refiere a "hoy", debes resolverlo a la fecha de hoy (ej: "${currentDateText} (1 día)"). Si se refiere a "mañana", debes resolverlo a la fecha de mañana (calculando el día calendario de mañana a partir de la fecha de hoy de la conversación; por ejemplo, si hoy es Jueves 6 de Agosto de 2026, debes retornar exactamente "Viernes 7 de Agosto de 2026 (1 día)"). Esto es de carácter obligatorio; tienes estrictamente prohibido retornar el término literal "hoy" o "mañana".
